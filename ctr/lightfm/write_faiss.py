...
#path = '../data/'
path = '/data/public_rsync/fm_basic/data/'
d = 64
nlist = 2
k = 1

user_embedding = np.loadtxt(open(path + "item_embedding.csv", "rb"), delimiter=",", skiprows=0)
user_embedding = np.array(user_embedding, dtype=np.float32)
print(user_embedding.shape)
print(type(user_embedding))

u_label_model_dict = joblib.load(path + 'i_label_model_dict.csv')
print(list(u_label_model_dict.keys())[0])
...
pickle.dump(ids_user_embedding, open(path + 'ids_vectors.p', 'wb'))
